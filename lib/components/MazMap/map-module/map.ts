import { createHTMLMarker, HTMLMarker } from './html-marker'
...
export class Map {
  ...
  constructor(private readonly mapContainerId: string, private readonly apiKey: string, private readonly options?: MapOptions) {
    this.mapOptions = {
      ...DEFAULT_OPTIONS,
      ...this.options,
    }
  ...
  public async init(options?: ScriptOptions): Promise<void> {
    try {
      ...
      this.map = new this.api.maps.Map(document.querySelector(`#${this.mapContainerId}`) as HTMLDivElement, this.mapOptions)
    } catch (error: any) {
  ...
  private getInfoWindow(options: MarkerInfoWindowOptions): google.maps.InfoWindow {
    if (options.offset) options.pixelOffset = new this.api.maps.Size(options.offset.x, options.offset.y)
    return new this.api.maps.InfoWindow({
      ...options,
    })
  }

  private addInfoWindowContentToMarker(marker: HTMLMarker | google.maps.Marker, infoWindowOptions: MarkerInfoWindowOptions) {
    if (!infoWindowOptions.content) throw new Error('[Map] InfoWindow should have a content')
