  ...
  export default defineComponent({
    ...
    props: {
      ...
      color: {
        ...
        default: 'primary',
        validator: (value: string) => {
          return ['primary', 'warning', 'secondary', 'danger', 'light', 'white', 'transparent', 'base'].includes(value)
        },
      },
      type: {
        type: String,
    ...
    setup(props, { attrs, slots }) {
      ...
      const cursorClass = computed(() => (isDisabled.value ? 'maz-cursor-default' : 'maz-cursor-pointer'))
      const variantClass = computed(() => `-is-${props.variant}`)
      const loaderBgColorClass = computed(() => `-${props.color}`)
      const loaderColor = computed(() => (['white', 'light', 'transparent'].includes(props.color) ? 'black' : 'white'))
