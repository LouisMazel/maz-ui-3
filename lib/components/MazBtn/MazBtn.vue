<template>
  <component
    :is="componentType"
    :id="id"
    v-bind="$attrs"
    class="maz-base-component maz-btn maz-inline-flex maz-p-4 maz-rounded"
    :class="[
      classes,
      {
        'maz-text-hidden': loading,
      },
    ]"
    :type="isLink ? null : type"
    :disabled="isLink ? null : isDisabled"
    @click="handleClick($event)"
    @mouseenter="emitMouseEnter($event)"
    @mouseleave="emitMouseLeave($event)"
    @focus="emitFocus($event)"
    @blur="emitBlur($event)"
  >
    <div
      v-if="hasLeftIcon() || hasMainIcon()"
      class="maz-flex maz-flex-center maz-btn__icon-left"
      :class="{
        'maz-mr-2': !fab && hasSlotDefault(),
      }"
    >
      <!-- Icon slot (`icon-left`) -->
      <slot :name="`icon-left` || `icon`">
        <!-- none -->
        <i class="material-icons">{{ leftIconName || iconName }}</i>
      </slot>
    </div>

    <span
      :class="[textClasses, { 'maz-flex-1': hasSlotDefault() }]"
      class="maz-flex maz-items-center maz-h-100 maz-overflow-hidden"
    >
      <slot />
    </span>

    <div
      v-if="hasRightIcon()"
      class="maz-flex maz-flex-center maz-btn__icon-right"
      :class="{
        'maz-ml-2': !fab && hasSlotDefault(),
      }"
    >
      <!-- Icon slot (`icon-right`) -->
      <slot :name="`icon-right`">
        <!-- none -->
        <i class="material-icons">{{ rightIconName }}</i>
      </slot>
    </div>
    <!-- Add your button text here -->
    <div v-if="loading" class="maz-btn__spinner maz-flex maz-flex-center">
      <MazSpinner :size="25" :color="color" />
    </div>
  </component>
</template>

<script lang="ts">
  import { defineComponent } from 'vue'
  import MazSpinner from './../MazSpinner/MazSpinner.vue'
  // import uniqueId from './../../mixins/uniqueId'

  /**
   * > Simple button component
   */

  export default defineComponent({
    name: 'MazBtn',
    components: {
      MazSpinner,
    },
    // mixins: [uniqueId],
    inheritAttrs: false,
    props: {
      // is the id of the button
      id: { type: String, default: null },
      // is color type (`primary` / `secondary` / `third` / `success` / `danger` / `grey` / `info` / `warning` / `light` / `dark` / `default` / `white` / `black`)
      color: {
        type: String,
        default: 'primary',
      },
      // is the button type (button, submit or something else)
      type: { type: String, default: 'button' },
      // button size (`xl`, `lg` / `md` / `sm` / `mini`)
      size: { type: String, default: 'md' },
      // is a `boolean` to show the loader & disable it
      loading: { type: Boolean, default: false },
      // is a `boolean` to disable the button
      disabled: { type: Boolean, default: false },
      // apply the outline style
      outline: { type: Boolean, default: false },
      // apply the rounded style
      rounded: { type: Boolean, default: false },
      // apply the fab style
      fab: { type: Boolean, default: false },
      // apply the focus style
      active: { type: Boolean, default: false },
      // take 100% of the width
      block: { type: Boolean, default: false },
      // remove shadow/elevation
      noShadow: { type: Boolean, default: false },
      // should be a [material icon](https://material.io/resources/icons/) name (usefull with fab buttons)
      iconName: { type: String, default: undefined },
      // should be a [material icon](https://material.io/resources/icons/) name
      leftIconName: { type: String, default: undefined },
      // should be a [material icon](https://material.io/resources/icons/) name
      rightIconName: { type: String, default: undefined },
      // align text to left (for block button)
      justifyStart: { type: Boolean, default: false },
      // align text to right (for block button)
      justifyEnd: { type: Boolean, default: false },
    },
    emits: ['input', 'click', 'mouseenter', 'mouseleave', 'focus', 'blur'],
    computed: {
      componentType(): string {
        const { href, to } = this.$attrs
        if (href) return 'a'
        else if (to) return 'router-link'
        return 'button'
      },
      isLink(): boolean {
        return this.componentType === 'a'
      },
      isDisabled(): boolean {
        const { disabled, loading } = this
        return loading || disabled
      },
      classes(): (string | undefined)[] {
        const {
          color,
          size,
          outline,
          rounded,
          fab,
          active,
          block,
          noShadow,
          hasRightIcon,
          hasLeftIcon,
          hasIcon,
          hasSlotDefault,
        } = this
        return [
          ...(color ? [`maz-btn--${color}`] : [undefined]),
          ...(size ? [`maz-btn--${size}`] : [undefined]),
          ...(outline ? ['maz-btn--outline'] : [undefined]),
          ...(rounded ? ['maz-btn--rounded'] : [undefined]),
          ...(block ? ['maz-btn--block'] : [undefined]),
          ...(fab ? ['maz-btn--fab'] : [undefined]),
          ...(active ? ['maz-active'] : [undefined]),
          ...(noShadow ? ['maz-no-shadow'] : [undefined]),
          ...(hasLeftIcon() ? ['maz-btn--icon--left'] : [undefined]),
          ...(hasRightIcon() ? ['maz-btn--icon--right'] : [undefined]),
          ...(hasIcon() ? ['maz-btn--icon'] : [undefined]),
          ...(!hasSlotDefault() ? ['maz-btn--no-text'] : [undefined]),
        ]
      },
      textClasses(): (string | undefined)[] {
        const { justifyStart, justifyEnd } = this
        return [
          ...(justifyStart ? ['maz-justify-start'] : [undefined]),
          ...(justifyEnd ? ['maz-justify-end'] : [undefined]),
          ...(!justifyStart && !justifyEnd ? ['maz-justify-center'] : [undefined]),
        ]
      },
    },
    methods: {
      hasSlotDefault() {
        return this.$slots['default']
      },
      hasIcon() {
        return this.hasLeftIcon() || this.hasRightIcon() || this.hasMainIcon()
      },
      hasMainIcon() {
        return this.iconName || this.$slots['icon']
      },
      hasLeftIcon() {
        return this.leftIconName || this.$slots['icon-left']
      },
      hasRightIcon() {
        return this.rightIconName || this.$slots['icon-right']
      },
      handleClick(event: Event) {
        // return click event
        this.$emit('click', event)
      },
      emitMouseEnter(event: Event) {
        // return mouseenter event
        this.$emit('mouseenter', event)
      },
      emitMouseLeave(event: Event) {
        // return mouseleave event
        this.$emit('mouseleave', event)
      },
      emitFocus(event: Event) {
        // return focus event
        this.$emit('focus', event)
      },
      emitBlur(event: Event) {
        // return blur event
        this.$emit('blur', event)
      },
    },
  })
</script>

<style lang="postcss" scoped>
  .maz-btn {
    @apply maz-bg-blue-800 maz-text-white;
  }
</style>
