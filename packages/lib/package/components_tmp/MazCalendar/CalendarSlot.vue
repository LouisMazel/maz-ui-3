  ...
  export default class CalendarSlot extends Vue {
    ...
    get slotText() {
      return this.showEndDateSlot
        ? `${this.$dayjs(this.timeSlot.start).format(
            this.format,
          )} - ${this.$dayjs(this.timeSlot.end).format(this.format)}`
    ...
    get isSelected() {
      return this.selectedSlots.some(
        ({ start, end }) =>
          this.isSameDate(this.timeSlot.start, start) &&
          this.isSameDate(this.timeSlot.end, end),
      )
    }
