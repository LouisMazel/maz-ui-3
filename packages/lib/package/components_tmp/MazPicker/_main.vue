...
<script lang="ts">
  ...
  export default {
    ...
    computed: {
      ...
      pickerTransition() {
        return this.calcPosition.includes('bottom')
          ? 'maz-slide'
          : 'maz-slideinvert'
      },
      hasHeader() {
    ...
    watch: {
      ...
      hasPickerOpen: {
        async handler(value) {
          ...
          if (value)
            this.calcPosition = this.position || `${verticalPosition} left`
        },
        immediate: true,
      },
    },
    ...
    methods: {
      ...
      emitFormatted(value) {
        if (this.value) {
          ...
          this.$emit(
            'formatted',
            getFormattedValue(value, this.format, this.formatted, this.range),
          )
        }
      },
      openPicker() {
      ...
      async getVerticalPosition() {
        ...
        let datePickerHeight = this.hasDouble
          ? DOUBLE_PICKER_HEIGHT
          : PICKER_HEIGHT

        datePickerHeight = this.noFooter
          ? datePickerHeight - HEADER_HEIGHT
          : datePickerHeight
        datePickerHeight = this.noHeader
          ? datePickerHeight - FOOTER_HEIGHT
          : datePickerHeight
        if (parentRect.top < datePickerHeight) {
          ...
          windowHeight -
            (parentRect.height + datePickerHeight + parentRect.top) >=
          0
        ) {
